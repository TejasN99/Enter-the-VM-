	...
	for (;;) {
		...
		case KVM_EXIT_IO:
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
			    ...
			    && vcpu->kvm_run->io.port == 0xF0) {
				out_count ++;
				char *p = (char *)vcpu->kvm_run;
				printf("%u\n", *(int *)(p + vcpu->kvm_run->io.data_offset));
				fflush(stdout);
				continue;
			}
			else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN
			    && vcpu->kvm_run->io.port == 0xF1) {
				in_count ++;
				char *p = (char *)vcpu->kvm_run;
				*(p + vcpu->kvm_run->io.data_offset) = exit_count;
			    ...
			    && vcpu->kvm_run->io.port == 0xF2) {
				out_count ++;
				char *p = (char *)vcpu->kvm_run;
				unsigned long x = *(unsigned long *)(p + vcpu->kvm_run->io.data_offset);
				printf("%s", (vm->mem + x));
				fflush(stdout);
				continue;
			} 
			else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
			    && vcpu->kvm_run->io.port == 0xF3) {
				in_count ++;
				char *p = (char *)vcpu->kvm_run;
				unsigned long x = *(unsigned long *)(p + vcpu->kvm_run->io.data_offset);
				char *arr = vm->mem + x;
			    ...
			    && vcpu->kvm_run->io.port == 0xF4) {
				in_count ++;

				char *p = (char *)vcpu->kvm_run;
				unsigned long x = *(unsigned long *)(p + vcpu->kvm_run->io.data_offset);
				struct args *arg = (struct args *)(vm->mem + x);

				struct kvm_translation translate;
				translate.linear_address = arg->gva;					
    			ioctl(vcpu->vcpu_fd, KVM_TRANSLATE, &translate);

				if (translate.valid == 0) {
					printf("Invalid GVA\n");
					arg->hva = 0;
				}
				else{
					arg->hva = translate.physical_address + (unsigned long)vm->mem; // This is My translation
				}
